...
const View = ({ type, data, createNew }: Props) => {
    ...
    const deleteFile = async (p: string, g: string, name: string, id: string) => {
        p && await UserAuthen.deleteFile(p, g, name, id)
        store.dispatch(setRefresh())
    }
            ...
            return (
                <div className='grid_box'>
                    ...
                    {data.map((item, index) =>
                        ...
                        >
                            <DeleteIcon style={{ position: "absolute", top: 5, right: 5, zIndex: 1, borderRadius: "5px" }} onClick={() => deleteFile(position, item.genre, item.name, item._id)} />
                            ...
                            <div style={{ aspectRatio: 10 / 2, margin: "5px 0px", fontSize: "0.9rem", textAlign: "center" }}>
                                <p title={item.name} style={{ width: "100%", overflow: "hidden", textOverflow: "ellipsis", textWrap: "nowrap" }}
                                    onClick={() => { item.slug ? toPage.push(item.genre + "/" + item.slug) : null }}>{item.name || item.title} </p>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    <div className='grid_box'>
                        ...
                        {data.map((item, index) =>
                            ...
                            >
                                ...
                                <div style={{ aspectRatio: 10 / 2, margin: "5px 0px", fontSize: "0.9rem", textAlign: "center" }}>
                                    <p title={item.name} style={{ width: "100%", overflow: "hidden", textOverflow: "ellipsis", textWrap: "nowrap" }}
                                        onClick={() => { item.slug ? toPage.push(item.genre + "/" + item.slug) : setViewFileModal(true), setFileId(item.name) }}>{item.title}</p>
                                </div>
                            </div>
                        )}
                    </div>
                    <PlayerView fileId={fileId} open={viewFileModal} closeModal={() => setViewFileModal(false)} />
                </>
            )
        case "grid":
            return (
