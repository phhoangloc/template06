...
const SingleView = ({ item, edit, archive }: Props) => {
    ...
    const updateItem = async (p: string, a: string, id: string, body: any) => {
        const newcover = fileCover && await UserAuthen.uploadFile(fileCover)
        ...
        edit ?
            <div className='grid_box scrollNone'>
                ...
                <div className={`detailBox xs12 md6 lg8 `} style={{ overflowX: "hidden" }}>
                    <Button name="save" onClick={() => createItem(item.position, archive, { name, slug, cover, detail })} />
            ...
            wannaEdit ?
                <div className='grid_box scrollNone'>
                    ...
                    <div className={`detailBox xs12 md6 lg8 `} style={{ overflowX: "hidden" }}>
                        <div style={{ display: "flex" }}><Button name="save" onClick={() => updateItem(item.position, archive, item._id, { name, slug, detail })} />
                            <Button name="cancle" onClick={() => setWannaEdit(false)} /></div>
                        <Input name="name" onChange={(e) => setName(e)} value={name} />
                        <Input name="slug" onChange={(e) => setSlug(e)} value={slug} />
                        <TextAreaTool name='' onChange={(e) => setDetail(e)} value={detail} />
                    </div>
                ...
                <div className='grid_box scrollNone'>
                    ...
                    <div className={`detailBox xs12 md6 lg8 `} style={{ margin: "10px", overflowX: "hidden" }}>
                        {currentUser?._id.toString() === item.host._id.toString() ? <Button name='edit' onClick={() => setWannaEdit(!wannaEdit)} /> : null}
